// Ipsilateral shock interaction problem
#include <iostream>
#include <cstdio>
#include <cstdlib>
#include <fstream>
    ...
    for(int rk = 0; rk < 3; rk++){
        ...
        for(int i = 5; i < m + 5; i++){
            for(int ng = 0; ng < 5; ng++){
                U[0][i][n+5+ng] = 1.0;
                U[1][i][n+5+ng] = 0.93969262;
                U[2][i][n+5+ng] = -0.34202014;
                U[3][i][n+5+ng] = 0.07936508 / (gamma_val -1) + 0.5*(0.93969262*0.93969262 + 0.34202014*0.34202014);
                }
            }
        // bottom
        for(int i = 5; i < m + 5; i++){
            for(int ng = 0; ng < 5; ng++){
                ...
                if(domain_x[0]+(i-5)*dx<2.0 && domain_x[0]+(i-5)*dx>0.5){
                    U[0][i][ng] = U[0][i][9-ng];
                    U[1][i][ng] = U[1][i][9-ng];
                    U[2][i][ng] = -U[2][i][9-ng];
                    U[3][i][ng] = U[3][i][9-ng];
                }
                if(domain_x[0]+(i-5)*dx>=2.0){
                    U[0][i][ng] = 1.0;
                    U[1][i][ng] = 1.0;
                    U[2][i][ng] = 0.0;
                    U[3][i][ng] = (1.0/1.4) / (gamma_val - 1) + 0.5;
                }
            }
        }
        ...
        for(int j = 5; j < n + 5; j++){
            for(int ng = 0; ng < 5; ng++){
                ...
                U[1][ng][j] = 0.93969262;
                U[2][ng][j] = -0.34202014;
                U[3][ng][j] = 0.07936508 / (gamma_val -1) + 0.5*(0.93969262*0.93969262 + 0.34202014*0.34202014);
            }
        }
        // right
        for(int j = 5; j < n + 5; j++){
...
double initialization_u(double x, double y){ return 1.0; }
