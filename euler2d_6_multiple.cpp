    ...
    for(int rk = 0; rk < 3; rk++){
        ...
        for(int i = 5; i < m + 5; i++){
            for(int ng = 0; ng < 5; ng++){
                U[0][i][n+5+ng] = U[0][i][n+4-ng];
                U[1][i][n+5+ng] = U[1][i][n+4-ng];
                U[2][i][n+5+ng] = -U[2][i][n+4-ng];
                U[3][i][n+5+ng] = U[3][i][n+4-ng];
                }
            }
        // bottom
        for(int i = 5; i < m + 5; i++){
            for(int ng = 0; ng < 5; ng++){
                if(domain_x[0]+(i-5)*dx<=2.0 && domain_x[0]+(i-5)*dx>=1.0/6.0){
                    U[0][i][ng] = 1.0;
                    U[1][i][ng] = 2.6108;
                    U[2][i][ng] = 0.0;
                    U[3][i][ng] = 1.0/1.4 / (gamma_val-1) + 0.5*2.6108*2.6108;
                }else{
                    U[0][i][ng] = U[0][i][9-ng];
                    U[1][i][ng] = U[1][i][9-ng];
                    U[2][i][ng] = -U[2][i][9-ng];
        ...
        for(int j = 5; j < n + 5; j++){
            for(int ng = 0; ng < 5; ng++){
                U[0][ng][j] = 2.6667;
                U[1][ng][j] = 1.6533*2.6667;
                U[2][ng][j] = 0;
                U[3][ng][j] = 3.2143 / (gamma_val -1) + 0.5*1.6533*1.6533;
            }
        }
        // right
        for(int j = 5; j < n + 5; j++){
...
//                    f_hat[k] = MRWENO(f_p[k][0], f_p[k][1], f_p[k][2], f_p[k][3], f_p[k][4]) + MRWENO(f_m[k][0], f_m[k][1], f_ m[k][2], f_m[k][3], f_m[k][4]);
...
double initialization_u(double x, double y){ return 2.6108; }
