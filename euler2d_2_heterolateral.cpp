// heterolateral shock interaction problem
#include <iostream>
#include <cstdio>
#include <cstdlib>
#include <fstream>
        ...
        for(int i = 5; i < m + 5; i++){
            for(int ng = 0; ng < 5; ng++){
                if(domain_x[0]+(i-5)*dx<=0.5){
                    ...
                    U[1][i][ng] = 4.0;
                    U[2][i][ng] = 0.0;
                    U[3][i][ng] = (1.0/1.4) / (gamma_val -1) + 0.5*16.0;
                }else{
                    U[0][i][ng] = 2.6667;
                    U[1][i][ng] = 2.6667*3.3750;
                    U[2][i][ng] = 2.6667*1.0825;
                    U[3][i][ng] = (3.2143) / (gamma_val -1) + 0.5*2.6667*(3.3750*3.3750 + 1.0825*1.0825);
                }
            }
        }
